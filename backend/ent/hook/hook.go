...
// The AccountUserBindingFunc type is an adapter to allow the use of ordinary
// function as AccountUserBinding mutator.
type AccountUserBindingFunc func(context.Context, *ent.AccountUserBindingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AccountUserBindingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AccountUserBindingMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AccountUserBindingMutation", m)
}
