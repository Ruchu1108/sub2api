...
// The AccountUserBindingFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountUserBindingFunc func(context.Context, *ent.AccountUserBindingQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountUserBindingFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountUserBindingQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountUserBindingQuery", q)
}

// The TraverseAccountUserBinding type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountUserBinding func(context.Context, *ent.AccountUserBindingQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountUserBinding) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountUserBinding) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountUserBindingQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountUserBindingQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountUserBindingQuery:
		return &query[*ent.AccountUserBindingQuery, predicate.AccountUserBinding, accountuserbinding.OrderOption]{typ: ent.TypeAccountUserBinding, tq: q}, nil
