...
type UserQuery struct {
	...
	withAccountBindings       *AccountUserBindingQuery
...
func (_q *UserQuery) QueryAccountBindings() *AccountUserBindingQuery {
...
func (_q *UserQuery) WithAccountBindings(opts ...func(*AccountUserBindingQuery)) *UserQuery {
	query := (&AccountUserBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withAccountBindings; query != nil {
		if err := _q.loadAccountBindings(ctx, query, nodes,
			func(n *User) { n.Edges.AccountBindings = []*AccountUserBinding{} },
			func(n *User, e *AccountUserBinding) { n.Edges.AccountBindings = append(n.Edges.AccountBindings, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadAccountBindings(ctx context.Context, query *AccountUserBindingQuery, nodes []*User, init func(*User), assign func(*User, *AccountUserBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountuserbinding.FieldUserID)
	}
