...
func TestOpenAIWSHTTPBridgePreviousResponseIDFullHistoryDeduplicatesReplay(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"text/event-stream"}}, Body: io.NopCloser(strings.NewReader(completed("resp_1", `[{"type":"custom_tool_call","id":"gateway_item","call_id":"call_1","name":"exec","input":"pwd"}]`)))},
		{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"text/event-stream"}}, Body: io.NopCloser(strings.NewReader(completed("resp_2", `[]`)))},
		{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"text/event-stream"}}, Body: io.NopCloser(strings.NewReader(completed("resp_3", `[]`)))},
	}}
	cfg := &config.Config{}
	...
	account := &Account{
		ID: 9004, Name: "oauth-previous-full-context", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
		Credentials: map[string]any{"access_token": "test-token"}, Extra: map[string]any{"responses_websockets_v2_enabled": true},
		Concurrency: 1, Status: StatusActive, Schedulable: true,
	}
	...
	writeAndRead(`{"type":"response.create","model":"gpt-5.1","input":"run pwd"}`)
	writeAndRead(`{"type":"response.create","model":"gpt-5.1","previous_response_id":"resp_1","input":[{"type":"input_text","text":"client history"},{"type":"custom_tool_call","id":"client_item","call_id":"call_1","name":"exec","input":"pwd"},{"type":"custom_tool_call_output","call_id":"call_1","output":"/tmp"}]}`)
	writeAndRead(`{"type":"response.create","model":"gpt-5.1","previous_response_id":"resp_2","input":[{"type":"input_text","text":"new client history"},{"type":"custom_tool_call","id":"new_client_item","call_id":"call_1","name":"exec","input":"pwd"},{"type":"custom_tool_call_output","call_id":"call_1","output":"/tmp"}]}`)
	require.NoError(t, clientConn.Close(coderws.StatusNormalClosure, "done"))
	...
	for idx, expectedItemID := range []string{"client_item", "new_client_item"} {
		input := gjson.GetBytes(upstream.bodies[idx+1], "input").Array()
		callCount := 0
		outputCount := 0
		for _, item := range input {
			...
			if isCodexToolCallContextItemType(item.Get("type").String()) {
				callCount++
				require.Equal(t, expectedItemID, item.Get("id").String())
			}
